...
function bluetag.new(style)
	...
	function widg:draw(_, cr, width)
		...
		if n > 0 then
			...
			for i = 1, n do
				-- if awful.screen.focused().index ~= data.state.list[i].screenindex then
				-- 	awful.spawn("notify-send " .. tonumber(awful.screen.focused().index))
				-- end
				
				local cl = data.state.list[i].focus and data.state.list[i].screenindex and beautiful.color.secondary or
