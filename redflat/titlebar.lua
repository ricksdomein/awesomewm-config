...
function titlebar.button.base(icon, style, is_inactive)
	...
	-- function widg:set_active(active)
	-- 	if active and awful.screen.focused().index ~= oldscreen then
	-- 		widg._current_color = style.color.main
	-- 	elseif awful.screen.focused().index ~= oldscreen then
	-- 		widg._current_color = style.color.secondary
	-- 	else
	-- 		widg._current_color = style.color.icon
	-- 	end
	...
	local oldscreen = awful.screen.focused().index
	function widg:set_active(active)
		if active and awful.screen.focused().index then
			widg._current_color = style.color.main
		elseif awful.screen.focused().index == oldscreen then
			widg._current_color = style.color.icon
		else
...
function titlebar.label(c, style, is_highlighted)
	...
	if is_highlighted then
		c:connect_signal("focus", function() oldscreen = awful.screen.focused().index; w._current_color = style.color.main; update() end)
		c:connect_signal("unfocus", function() 
			if awful.screen.focused().index == oldscreen then
			w._current_color = style.color.text 
			else
				w._current_color = style.color.secondary
			end
			oldscreen = awful.screen.focused().index
			update()end)
