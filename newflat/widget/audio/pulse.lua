...
function pulse:change_volume(args)
	...
	local m = redutil.read.output(string.format("pactl -- get-sink-mute " .. sink))
	...
	local v = redutil.read.output(string.format("pactl -- get-sink-volume " .. sink))
	...
	-- set new volume
	awful.spawn(string.format("pactl -- set-sink-volume " .. sink .. " %s%%", new_volume))
...
function pulse:mute(args)
	local sink = redutil.read.output(string.format("pactl get-default-sink"))
	awful.spawn(string.format("pactl -- set-sink-mute " .. sink .. " toggle"))
	-- initialize vars
	args = redutil.table.merge(change_volume_default_args, args or {})
	local diff = args.down and -args.step or args.step
...
function pulse:update_volume(args)
	...
	local mute = not (m and string.find(m, "false"))
