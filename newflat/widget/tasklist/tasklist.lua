...
local function get_state(c_group, style)
	...
	local state = { focus = false, urgent = false, minimized = true, list = {}, screenindex = 0 }
	...
	for _, c in pairs(c_group) do
		...
		state.screenindex = state.screenindex or c.screen.index

		local focus_oldscreen = false

		if awful.screen.focused().index ~= c.screen.index and awful.client.focus.history.get(c.screen.index, 0) == c then 
			focus_oldscreen = true
		end
		-- oldscreen = awful.screen.focused().index


		-- awful.spawn("notify-send " .. tonumber(c.screen.index))
		table.insert(state.list, { focus = client.focus == c or focus_oldscreen, urgent = c.urgent, minimized = c.minimized, screenindex = focus_oldscreen })
	end
...
local function tagline_construct(setup_layout, style)
	...
	local columns = math.ceil(#tags / style.tagline.rows)
...
local function new_task(c_group, style)
	...
	task.widg:connect_signal("mouse::leave",
		function()
			redtasklist.tasktip.hidetimer:emit_signal("timeout")
...
local function tasktip_line(style)
	...
	line.tb:set_font(beautiful.fonts.tooltip)
	...
	function line:mark_focused()
		line.field:set_bg(style.color.color)
