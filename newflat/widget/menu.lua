...
function menu:add(args)
	...
	if type(args[1]) ~= "string" and args.widget then
		...
		if args.select == true then
			local theme = redutil.table.merge(self.theme, args.theme or {})
			args.theme = theme
			args.new = args.new or menu.entry

			args._background = wibox.container.background(args.widget)
			args._background:set_bg("transparent")

			table.insert(self.items, args)
			self.layout:add(args._background)
			...
			args._background:buttons(awful.util.table.join(
				awful.button({}, 3, function () self:hide() end),
				awful.button({}, 1, press_action, release_action)
			))

			args.widget:connect_signal("mouse::enter", function() self:item_enter(num, { hover = true }) end)
		else
...
function menu.entry(parent, args)
	...
	args.cmd  = args[2] or args.cmd or ""
